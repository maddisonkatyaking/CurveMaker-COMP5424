...
class CurveMakerWidget:
  ...
  def setup(self):
	...
	#remove curves
	removeCollapsibleButton = ctk.ctkCollapsibleButton()
	removeCollapsibleButton.text = "Remove Curve"
	self.layout.addWidget(removeCollapsibleButton)
	removeFormLayout = qt.QFormLayout(removeCollapsibleButton)


	self.removeButton = qt.QPushButton("Remove Curve")
	self.removeButton.toolTip = "Remove Curve"
	self.removeButton.name = "Remove Curve"
	removeFormLayout.addWidget(self.removeButton)
	self.removeButton.connect('clicked()', self.onRemoveButton)

	#merge curves
	# giving it a red hot go
	mergeCollapsibleButton = ctk.ctkCollapsibleButton()
	mergeCollapsibleButton.text = "Merge Curves"
	self.layout.addWidget(mergeCollapsibleButton)
	mergeFormLayout = qt.QFormLayout(mergeCollapsibleButton)
	
	#merge button
	self.mergeButton = qt.QPushButton("Merge Curve")
	self.mergeButton.toolTip = "Merge curves."
	self.mergeButton.name = "Merge Curves"
	mergeFormLayout.addWidget(self.mergeButton)
	self.mergeButton.connect('clicked()', self.onMergeButton)
